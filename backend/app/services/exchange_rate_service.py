...
async def fetch_and_store_exchange_rates(db: Session):
    try:
        ...
        for quote_key, rate_value in quotes.items():
            ...
            existing_rate = (
                ...
                .filter(
                    ...
                    ExchangeRate.timestamp >= quoted_at - timedelta(minutes=1),
                )
                .first()
            )
            ...
            if not existing_rate:
                exchange_rate = ExchangeRate(
                    currency_pair_id=currency_pair.id, rate=rate_value, timestamp=quoted_at
                )
        ...
        logger.info("Stored %d rates quoted at %s", stored_count, quoted_at.isoformat())

        return {
            "message": f"Successfully fetched and stored {stored_count} exchange rates",
            "base_currency": base_currency,
            "timestamp": quoted_at.isoformat(),
